...
fn invert_color(color: &str) -> Result<String, ParseColorError> {
    ...
    let result = format!(
        "#{:02X}{:02X}{:02X}",
        inv_components[0], inv_components[1], inv_components[2]
    );
    Ok(result)
}
...
mod tests {
    ...
    fn test_invert_color() {
        ...
        assert_eq!(invert_color("#FFFFFF").unwrap(), "#000000");
        assert_eq!(invert_color("#FF0000").unwrap(), "#00FFFF");
        assert_eq!(invert_color("#00FF00").unwrap(), "#FF00FF");
        assert_eq!(invert_color("#0000FF").unwrap(), "#FFFF00");
        assert_eq!(invert_color("#FF00FF").unwrap(), "#00FF00");
        assert_eq!(invert_color("#00FFFF").unwrap(), "#FF0000");
        assert_eq!(invert_color("#FFFF00").unwrap(), "#0000FF");
    }
